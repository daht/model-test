...
### 5.2 2026-07-29 Qwen3-TTS 单并发实测

服务使用高并发 deploy 配置、stage 1 `max_num_seqs=1`，并在单卡上把两个 stage 固定到 device 0。客户端通过公网 WebSocket 独立发压，服务端同时采集 GPU 和容器监控。

本轮服务端监控证据已持久化为：`log/20260729T025911Z-beee05.tar.gz`。

新 ramp 与容量阶梯的服务端监控归档：`log/20260729T051309Z-fb52c3.tar.gz`。该轮配置为 `codec_chunk_frames: 25`、`codec_chunk_ramp: [6, 8, 12, 18, 25]`、stage 1 `max_num_seqs: 1`。

| 轮次 | 成功/请求 | TTFA p95 | chunk gap p99 | 播放断流 | 结论 |
|---|---:|---:|---:|---:|---|
| 30 秒预检 | 9/9 | 116 ms | 425 ms | 9/9，累计 2.508 秒 | 首包和块间隔达标，连贯性不达标 |
| 120 秒单并发 | 32/32 | 103 ms | 877 ms | 32/32，累计 9.598 秒 | 断流稳定复现，不能进入容量定档 |
| 新 ramp 30 秒预检 | 9/9 | 165 ms | 1.187 秒 | 0/0 秒 | 连贯性修复；样本量不足，继续 120 秒复核长尾 |
| 新 ramp 120 秒单并发 | 32/32 | 179 ms | 555 ms | 0/0 秒 | 通过单并发生产门槛，进入闭环并发阶梯 |
| 新 ramp 闭环并发 2，120 秒 | 54/54 | 365 ms | 692 ms | 0/0 秒 | 通过 |
| 新 ramp 闭环并发 3，120 秒 | 71/71 | 327 ms | 711 ms | 0/0 秒 | 通过，当前单实例闭环候选 |
| 新 ramp 闭环并发 4，120 秒 | 84/84 | 436 ms | 1.248 秒 | 0/0 秒 | gap p99 超标，停止升档并补测并发 3 |
| 新 ramp 开环 0.45 RPS，120 秒 | 66/66 | 627 ms | 1.149 秒 | 0/0 秒 | gap p99 超标 |
| 新 ramp 开环 0.55 RPS，120 秒 | 64/64 | 457 ms | 1.120 秒 | 0/0 秒 | gap p99 超标 |
| 新 ramp 开环 0.60 RPS，120 秒 | 76/76 | 446 ms | 966 ms | 0/0 秒 | 通过，需延长复测 |
| 新 ramp 开环 0.60 RPS，300 秒 | 190/190 | 532 ms | 1.465 秒 | 0/0 秒 | 延长复测失败，不进入 soak |
| 新 ramp 开环 0.30 RPS，300 秒 | 99/99 | 306 ms | 968 ms | 0/0 秒 | 通过，进入 30 分钟 soak |
| 新 ramp 开环 0.30 RPS，30 分钟 | 569/569 | 340 ms | 1.110 秒 | 0/0 秒 | 严格 gap SLO 失败，不可定为生产容量 |
| 稳态 16 帧，30 秒预检 | 9/9 | 204 ms | 762 ms | 0/0 秒 | 通过，进入 300 秒开环对照 |
| 稳态 16 帧，开环 0.30 RPS，300 秒 | 99/99 | 314 ms | 694 ms | 0/0 秒 | 通过，进入 30 分钟 soak |
| 稳态 16 帧，开环 0.30 RPS，30 分钟 | 569/569 | 337 ms | 651 ms | 0/0 秒 | 通过，作为生产候选并重新定并发容量 |
| 稳态 16 帧，闭环并发 1，120 秒 | 30/30 | 158 ms | 374 ms | 0/0 秒 | 通过 |
| 稳态 16 帧，闭环并发 2，120 秒 | 52/52 | 266 ms | 686 ms | 0/0 秒 | 通过 |
| 稳态 16 帧，闭环并发 3，120 秒 | 66/66 | 312 ms | 527 ms | 0/0 秒 | 通过 |
| 稳态 16 帧，闭环并发 4，120 秒 | 78/78 | 388 ms | 709 ms | 0/0 秒 | 通过 |
| 稳态 16 帧，闭环并发 5，120 秒 | 66/69 | 607 ms | 6.306 秒 | 3/7.585 秒 | 失败，3 个长文本 WebSocket 异常 |
| 稳态 16 帧，开环 0.50 RPS，120 秒 | 53/53 | 831 ms | 3.146 秒 | 1/0.394 秒 | gap 与断流超标 |
| 稳态 16 帧，开环 0.55 RPS，120 秒 | 64/64 | 531 ms | 1.060 秒 | 1/0.136 秒 | gap 与断流超标 |
| 稳态 16 帧，开环 0.60 RPS，120 秒 | 76/76 | 605 ms | 1.605 秒 | 0/0 秒 | gap 超标 |
| 稳态 16 帧，开环 0.40 RPS，300 秒 | 117/117 | 428 ms | 1.234 秒 | 0/0 秒 | gap p99 超标，不进入 soak |
| 稳态 16 帧，开环 0.35 RPS，300 秒 | 106/106 | 406 ms | 1.223 秒 | 0/0 秒 | gap p99 超标，不进入 soak |
| 稳态 16 帧，开环 0.325 RPS，300 秒 | 102/102 | 369 ms | 2.219 秒 | 4/1.332 秒 | gap 与断流超标，不进入 soak |

120 秒结果按语料桶拆分后，short、medium、long 分别为 11/11、11/11、10/10 请求断流；每次断流约 0.3 秒。当前首批 codec chunk 到达很快，但首段可播放音频不足以覆盖下一块生成间隔，因此播放器缓冲会短暂归零。原始客户端证据保存在：

- `log/tts-qwen-ramp-preflight-20260729/`
- `log/tts-qwen-ramp-c1-120s-20260729/`

`codec_chunk_ramp: [6, 8, 12, 18, 25]` 已通过 120 秒单并发验证。该参数增加首段播放缓冲，同时保留末项 25 与 `codec_chunk_frames` 对齐，避免 ramp 结束后重新出现 chunk-size cliff。新参数把每次约 0.3 秒的断流降至 0，同时 TTFA p95 保持在 179 ms。客户端原始证据保存在：

- `log/tts-qwen-ramp-6-8-12-18-25-preflight-20260729/`
- `log/tts-qwen-ramp-6-8-12-18-25-c1-120s-20260729/`

单并发已通过 `TTFA p95 < 1s`、`chunk gap p99 < 1s`、0 失败且 0 播放断流四项门槛，闭环并发 2 和 3 也通过。并发 4 的 chunk gap p99 为 1.248 秒，因此不再升到并发 8。开环 120 秒初扫中，只有 0.60 RPS 通过，但泊松到达下的 0.45 和 0.55 RPS 出现了偶发超过 1 秒的 gap；0.60 RPS 的 300 秒复测也出现 gap p95 1.171 秒、p99 1.465 秒。到达率降至 0.30 RPS 后，300 秒复测达到 TTFA p95 306 ms、gap p99 968 ms、0 失败、0 断流；但 30 分钟 soak 的 569 个请求中，gap p99 回升至 1.110 秒，最大值 2.120 秒。当前配置保持 0 断流，但不能满足严格的块间隔 SLO，也不能定为生产容量。下一轮不再只调首段 ramp，而是下调 `codec_chunk_frames` 并令 `codec_chunk_ramp` 的最后一项与该稳态 chunk 大小一致，目标是降低 Stage 1 稳态解码长尾。

下一轮候选参数为 `codec_chunk_frames: 16`、`codec_chunk_ramp: [6, 8, 12, 16]`，并在 `decode_cudagraph_capture_sizes` 中保留原有桶且增加 `16`。这是一次只改变稳态 chunk 大小及其直接图捕获依赖的对照实验；先跑 30 秒预检和 300 秒开环 0.30 RPS，再决定是否继续并发或 soak。

稳态 16 帧在 300 秒开环 0.30 RPS 下通过：TTFA p95 314 ms、gap p99 694 ms、0 失败、0 断流；相较旧稳态 25 帧同口径的 gap p99 968 ms 有明显改善。下一步在相同到达率执行 30 分钟 soak；通过后再重新测量闭环并发和更高开环到达率。

稳态 16 帧已通过 30 分钟 soak：569/569 成功，TTFA p95 337 ms、gap p99 651 ms、最大 gap 738 ms、0 失败、0 断流。相同 0.30 RPS、30 分钟口径下，旧稳态 25 帧的 gap p99 为 1.110 秒，因此 16 帧是当前生产候选。后续从闭环并发 1、2、3、4 重新定容量；每档 120 秒，首次违反任一门槛即停止升档，随后在最后通过档附近执行更长的开环复测。

闭环并发 1 至 4 均通过；并发 5 的 69 个请求中 3 个长文本发生 WebSocket 异常，错误率 4.348%，gap p99 6.306 秒，累计播放断流 7.585 秒。因此当前单实例闭环生产并发上限为 4。下一步以 0.50、0.55、0.60 RPS 执行开环初扫，并在最后通过点延长至 300 秒后定稳定到达率。

开环 0.50、0.55、0.60 RPS 初扫均未通过，原因是随机到达突发下的 gap 超标，0.50 和 0.55 RPS 还发生了播放断流。不能以闭环并发 4 的吞吐推断生产到达率；已知 0.30 RPS 的 30 分钟 soak 通过。0.40 RPS 的 300 秒复测为 117/117 成功、TTFA p95 428 ms、gap p99 1.234 秒、最大 gap 1.255 秒、0 断流；0.35 RPS 为 106/106 成功、TTFA p95 406 ms、gap p99 1.223 秒、最大 gap 1.337 秒、0 断流；0.325 RPS 为 102/102 成功、TTFA p95 369 ms、gap p99 2.219 秒、最大 gap 2.292 秒，并有 4 个播放断流请求、累计 1.332 秒。三个档位均未满足严格 SLO，因而不进入 soak。当前生产定档保持闭环并发上限 4、开环稳定到达率 0.30 RPS；后者已通过 30 分钟 soak，任何更高到达率均需在参数变更后重新验证。
...
当前状态：`app/tts_api.py` 已支持 MiniMax 风格事件、hex 兼容模式及 binary PCM 扩展，`app/tts.py` 已支持 CosyVoice 逐块生成；现有全局锁仍会串行化多个模型推理。`scripts/benchmark_tts_stream.py` 已支持 WS/WSS TTFA、chunk gap、播放断流、闭环并发和开环到达率，`scripts/monitor_tts_benchmark.sh` 已支持 GPU、容器及服务日志采集。尚缺服务端真实排队时间和自动音质评分；Qwen3-TTS 与 Kokoro 也尚未实现真流式适配器。
