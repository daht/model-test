...
下一轮先测试 `codec_chunk_ramp: [6, 8, 12, 18, 25]`。该参数增加首段播放缓冲，同时保留末项 25 与 `codec_chunk_frames` 对齐，避免 ramp 结束后重新出现 chunk-size cliff。参数变更后必须重新执行 30 秒预检和 120 秒单并发；只有 `TTFA p95 < 1s`、`chunk gap p99 < 1s`、0 失败且 0 播放断流全部满足，才进入闭环并发阶梯。
