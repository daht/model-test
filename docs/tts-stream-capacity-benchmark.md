...
2026-07-29 的 Qwen3-TTS 稳态 16 帧配置已完成当前一轮生产化验证。`0.30 RPS` 开环 30 分钟 soak 通过，闭环并发 1 至 4 通过；闭环并发 5 失败，`0.325/0.35/0.40 RPS` 开环复测均因 `chunk gap p99` 或播放断流超标失败。当前定档保持闭环并发上限 4、开环稳定到达率 `0.30 RPS`。详细证据与判定见 [Qwen3-TTS 单并发实测](tts-a10-websocket-streaming-test-plan.md#52-2026-07-29-qwen3-tts-单并发实测)。
...
### 5.1 2026-07-29 Qwen3-TTS 稳态 16 帧监控归档校验

本轮完整服务端监控归档为 `log/20260729T070500Z-9c0d3a.tar.gz`，SHA-256 为 `4e251e06c29520df6f8984ba6c7badc7a6b905844d774428df5002693324d7d6`。归档内 `manifest.sha256` 校验全部通过，`service.log` 非空，`collector-errors.log` 为空。

监控时间范围为 `2026-07-29T07:05:00.648Z` 到 `2026-07-29T12:21:29.687Z`，覆盖本轮稳态 16 帧 0.30 RPS soak、闭环并发阶梯和 0.325/0.35/0.40 RPS 开环复测。容器 `cosyvoice-tts-api` 在归档结束时仍为 running，`restart_count=0`，`oom_killed=false`，代码版本为 `f7fb049cc251d8f6960c1d29650839a34bd0f43e`。

监控报告记录 GPU 样本 36127 条、容器样本 9397 条。GPU 利用率均值/峰值为 `19.07%/100.00%`，显存使用均值/峰值为 `9251.65/9254.00 MiB`，功耗均值/峰值为 `77.53/159.10 W`，温度均值/峰值为 `53.50/68.00 C`。容器 CPU 均值/峰值为 `0.57%/6.83%`，PIDs 峰值为 38。

`service.log` 中 `08:01:35Z` 到 `08:01:47Z` 出现 3 组 `keepalive ping failed` / `data transfer failed` / `AssertionError`，与闭环并发 5 的 3 个长文本 WebSocket 异常一致；这支持并发 5 失败判定。压测结束后还出现公网扫描请求：`GET /../../../../../../etc/passwd`、`GET /` 和无效 HTTP 请求；这不是压测错误，但说明 adapter 端口曾直接暴露公网，生产入口应限制到反向代理或负载机来源。
